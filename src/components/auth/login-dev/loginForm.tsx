...
export default function RestrictedDevLogin() {
  ...
  const handleSubmit = async (values: FormValues, { setSubmitting }: SubmitHelpers) => {
    try {
      ...
      const errorMessage = errorObj.response?.data?.message || "Authentication failed";
    ...
    <div className="flex min-h-screen bg-gray-100">
      <div className="m-auto w-full max-w-md rounded-md bg-white p-6 shadow-md">
        <div className="mb-6">
          ...
          <h2 className="text-center text-xl font-semibold" style={{ color: "#0D3880" }}>Developer Access</h2>
          <div className="mt-1 flex items-center justify-center">
            <span className="inline-block h-1 w-6" style={{ backgroundColor: "#E60278" }}></span>
          </div>
          <p className="mt-2 text-center text-sm text-gray-600">Restricted portal entry</p>
        </div>

        <div className="mb-4 rounded border border-gray-200 bg-gray-50 p-3 text-xs text-gray-700">
          <p>This portal is restricted to authorized developer access only. Please authenticate with your credentials.</p>
        </div>
            ...
            <Form className="space-y-4">
              ...
              <div className="mt-6 border-t border-gray-200 pt-4">
                <p className="text-center text-xs text-gray-500">
                  <span style={{ color: "#E60278" }}>Developer Portal</span> - Authorized access only
                </p>
              </div>
            </Form>
          )}
