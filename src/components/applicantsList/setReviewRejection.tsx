...
export default function SetReviewRejection({ jobId, userId }: IProps) {
  ...
  return (
    <>
      <button type="button" onClick={menuHandler} className='px-2 py-1 font-medium text-white bg-red-500 text-xs rounded-lg'>REVIEW</button>
      ...
      <div className={`${open ? 'scale-100' : 'scale-0'} w-[50%] py-5 px-6 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 transition duration-300 bg-white z-[60] ${hidden ? '' : 'hidden'}`}>
        ...
        <textarea
          ...
          id="rejected-review"
          className="w-full h-full border outline-none border-black px-2 py-1 rounded-lg"
        ...
        />
        <button type="button" disabled={rejectedReview == ''} onClick={handleSave} className={`${rejectedReview == '' ? 'disabled:cursor-not-allowed' : 'hover:shadow-sm'} shadow-md border rounded-lg border-lightBlue shadow-lightBlue font-[550] mt-8 py-2 transition duration-300 w-full`}>SAVE</button>
      </div>
    </>
  )
}