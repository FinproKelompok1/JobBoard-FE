...
export default function JobsTable({ sort, search }: IProps) {
  const { data: jobs = [], isLoading, isValidating } = useSWR<IJob[]>(`/jobs?${sort}&${search}`, getJobs, { revalidateOnFocus: false, revalidateIfStale: false, revalidateOnReconnect: false });
  const skeletons = useMemo(() => Array.from({ length: 5 }), []);

  const handleDelete = async (jobId: string) => {
    try {
      // mutate(`/jobs?${sort}&${search}`, jobs.filter((job) => job.id != jobId), false)
      // mutate('/jobs/total', (total: number = 0) => total - 1, false)
                ...
                <tr key={item.id}>
                  <td>
                    <label className="inline-flex items-center cursor-pointer">
                      <input type="checkbox" onClick={() => handlePublish(item.id, !item.isPublished)} value={`${item.isPublished}`} className="sr-only peer" checked={item.isPublished} />
